...
<head>
    ...
    <style>
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .container { width: 100%; max-width: 800px; margin: auto; background: white; padding: 15px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        ...
        .sig-wrapper { position: relative; width: 100%; height: 250px; background: #fff; border: 2px dashed #bbb; border-radius: 8px; touch-action: none; }
        #signature-pad { width: 100%; height: 100%; }
        .canvas-container-outer { width: 100%; margin-top: 15px; background: #fff; border: 3px solid #000000; position: relative; touch-action: none; }
        .controls { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; margin: 15px 0; }
        button { flex: 1; min-width: 120px; padding: 12px; font-size: 16px; font-weight: bold; cursor: pointer; border: none; border-radius: 6px; }
...
<div class="container">
    ...
    <div class="color-selector">
        ...
        <div class="color-dot" style="background: #28C8C8;" data-color="#28C8C8"></div>
    ...
    <div class="sig-wrapper"><canvas id="signature-pad"></canvas></div>
    <div class="controls">
    ...
    <div id="step-2-area">
        ...
        <div class="canvas-container-outer" id="canvas-holder"><canvas id="final-canvas"></canvas></div>
...
<script>
    ...
    const signaturePad = new SignaturePad(sigCanvas, { backgroundColor: 'rgba(255,255,255,0)', penColor: '#000000' });
    ...
    // 初始化 Fabric 畫布
    const fabricCanvas = new fabric.Canvas('final-canvas', { stopContextMenu: true });
    ...
    function loadBackground() {
        ...
        .then(res => res.json())
        .then(data => {
            currentBgTimestamp = data.lastModified;
    ...
    document.getElementById('confirm-sign').addEventListener('click', () => {
        ...
        fabric.Image.fromURL(signData, (img) => {
            ...
            img.set({ 
                ...
                transparentCorners: false, borderColor: '#3498db' 
            });
            fabricCanvas.add(img);
            fabricCanvas.setActiveObject(img);
        });
        setTimeout(() => { window.scrollTo({ top: document.getElementById('step-2-area').offsetTop, behavior: 'smooth' }); }, 300);
    });
    ...
    document.getElementById('save-final').addEventListener('click', () => {
        ...
        .then(res => res.json())
        .then(data => {
            if (data.lastModified !== currentBgTimestamp) {
                alert("⚠️ 偵測到底圖已更新（可能有其他人剛簽完名），請重新確認您的簽名位置！");
                loadBackground(); 
                return;
            }
            ...
            const finalData = fabricCanvas.toDataURL({ format: 'png', quality: 1, multiplier: exportMultiplier });
